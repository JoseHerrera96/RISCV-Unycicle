...
# Verilog sources
SRC_FILES = top.v adder.v alu.v aludeco.v control_unit.v data_mem.v datapath.v imm_ext.v instr_mem.v maindeco.v mux4.v mux21.v pc_register.v register_file.v RISCVunicycle.v

# Testbenches
TB_RTL    = system/tb_top_rtl.v
TB_GLS    = unit/tb_counter_gls.v
...
# ============================================================================
.PHONY: help sim-rtl waves-rtl synth sim-gls waves-gls sim-unit waves-unit clean
...
sim-rtl: $(OUT_DIR)
	...
	vlib $(OUT_DIR)/work
	vlog -sv -work $(OUT_DIR)/work $(addprefix $(SRC_DIR)/,$(SRC_FILES)) $(TB_DIR)/$(TB_RTL)
	vsim -c -do "vcd file $(OUT_DIR)/$(VCD_RTL); vcd add -r /*; run -all; quit -f" \
		-lib $(OUT_DIR)/work tb_top_rtl
	@echo ""
...
synth: $(OUT_DIR)
	...
	$(STA) $(TB_DIR)/config/generate_sdf.tcl
	@echo ""
...
sim-unit: $(OUT_DIR)
	@if [ -z "$(TB)" ]; then echo "Error: set TB=unit/<tb>.v"; exit 1; fi; \
	echo "=== [SIM-UNIT] Running unit testbench $(TB) ==="; \
	vlib $(OUT_DIR)/work; \
	vlog -sv -work $(OUT_DIR)/work $(addprefix $(SRC_DIR)/,$(SRC_FILES)) $(TB_DIR)/$(TB); \
	vsim -c -do "vcd file $(OUT_DIR)/dump_unit.vcd; vcd add -r /*; run -all; quit -f" \
		-lib $(OUT_DIR)/work $$(basename $$(notdir $(TB:.v=)))

waves-unit:
	@if [ -f "$(OUT_DIR)/dump_unit.vcd" ]; then \
		$(GTKWAVE) $(OUT_DIR)/dump_unit.vcd >/dev/null 2>&1 & \
	else \
		echo "Error: unit VCD not found."; exit 1; \
	fi
